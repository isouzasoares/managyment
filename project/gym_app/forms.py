...
from gym_app.models import Athlete, Exercise, PersonalTrainer, BodyScreening
...
class AthleteForm(forms.ModelForm):
    ...
    level = forms.ChoiceField(widget=forms.Select(attrs={'class': "form-control"}),
                              choices=Athlete.LEVELS)
    training_period = forms.ChoiceField(widget=forms.Select(attrs={'class': "form-control"}), 
                                        choices=Athlete.TRAINING_PERIOD)
    gender = forms.ChoiceField(widget=forms.Select(attrs={'class': "form-control"}), 
                               choices=Athlete.GENDERS)
...
class BodyScreeningForm(forms.ModelForm):
    class Meta:
        model = BodyScreening
        fields = ('triceps', 'biceps', 'subscapular', 'supraspinale',
                  'suprailic', 'abdominal', 'chest', 'thigh', 'calf',
                  'weight', 'feet', 'inches')
